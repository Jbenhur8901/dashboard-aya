...
-- Helper function to check if current user is admin_fin or superadmin
...
-- Helper function to check if current user is admin, admin_fin, or superadmin
...
-- Helper function to check if current user is active (approved + not disabled)
...
-- Admins can update users (approve/disable/change role)
...
-- Admin/Fin can view all clients
CREATE POLICY "Clients: Admin/Fin can view"
  ON public.clients FOR SELECT
  USING (public.is_admin_or_fin());

-- Users can view clients (active users only)
...
-- Users can view souscriptions (active users only)
...
-- Users can update souscriptions (active users only)
CREATE POLICY "Souscriptions: Active users can update"
  ON public.souscriptions FOR UPDATE
  USING (public.is_active_user());
...
-- Users can view documents (active users only)
...
-- Users can view souscription_auto (active users only)
...
-- Users can view souscription_voyage (active users only)
...
-- Users can view souscription_mrh (active users only)
...
-- Users can view souscription_iac (active users only)
